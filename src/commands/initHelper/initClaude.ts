...
const CLAUDE_STOP_HOOK: ClaudeHookEntry = {
  matcher: '',
  hooks: [{ type: 'command', command: HOOK_COMMANDS.CLAUDE_STOP, timeout: 10 }],
};

const CLAUDE_END_HOOK: ClaudeHookEntry = {
  matcher: '',
  hooks: [{ type: 'command', command: HOOK_COMMANDS.CLAUDE_POST, timeout: 10 }],
};
...
async function ensureClaudeMd(paths: ClaudePaths): Promise<boolean> {
  ...
  if (await fileContainsLine(paths.claudeMdPath, AGENTS_ANCHOR_LINE)) {
    return false;
  }
