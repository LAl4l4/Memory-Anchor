...
test('incremental refresh persists forward and reverse edges and rerenders dirty target charts', async () => {
  ...
  const registryPath = path.join(tempDir, '.memoryanchor', 'dirTree.json');
  const initialProviderChars = JSON.parse(await readFile(registryPath, 'utf8'))
    .children.find(node => node.directory === 'providers').thisDirectoryChars;
  const initialGraph = JSON.parse(await readFile(graphPath, 'utf8'));
  ...
  const providerWithCallerChars = JSON.parse(await readFile(registryPath, 'utf8'))
    .children.find(node => node.directory === 'providers').thisDirectoryChars;
  ...
  const providerWithoutCallerChars = JSON.parse(await readFile(registryPath, 'utf8'))
    .children.find(node => node.directory === 'providers').thisDirectoryChars;
...
test('creating a dependency target refreshes unchanged importer charts', async () => {
  ...
  await buildPartitionedChartsForDebug(options);
  const registryPath = path.join(tempDir, '.memoryanchor', 'dirTree.json');
  const initialConsumerChars = JSON.parse(await readFile(registryPath, 'utf8'))
    .children.find(node => node.directory === 'consumers').thisDirectoryChars;
  ...
  const resolvedConsumerChars = JSON.parse(await readFile(registryPath, 'utf8'))
    .children.find(node => node.directory === 'consumers').thisDirectoryChars;
  ...
  await expect(readFile(consumerChartPath, 'utf8'))
    .resolves.not.toContain('-> ../providers/target.ts');
  const unresolvedConsumerChars = JSON.parse(await readFile(registryPath, 'utf8'))
    .children.find(node => node.directory === 'consumers').thisDirectoryChars;
...
test('graph-only importer size changes rebuild the affected topology boundary', async () => {
  ...
  const registryPath = path.join(tempDir, '.memoryanchor', 'dirTree.json');
  const consumerChars = JSON.parse(await readFile(registryPath, 'utf8'))
    .children.find(node => node.directory === 'consumers').thisDirectoryChars;
  const options = {
    projectRoot: tempDir,
