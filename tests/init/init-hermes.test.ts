...
function runInitHermes(cwd: string): Promise<void> {
...
async function readHermesConfig(cwd: string): Promise<ReturnType<typeof parseDocument>> {
  return parseDocument(
    await readFile(path.join(cwd, 'hermes-home', 'config.yaml'), 'utf8'),
  );
}

function getHookItems(
  doc: ReturnType<typeof parseDocument>,
  eventName: string
) {
  const list = doc.getIn(['hooks', eventName]);
  return isSeq(list) ? list.items : [];
}
...
function listCommands(doc: ReturnType<typeof parseDocument>, eventName: string): unknown[] {
  return getHookItems(doc, eventName).map(toJson);
}
...
test('does not register a prompt hook by default', async () => {
  ...
  expect(getHookItems(doc, 'pre_llm_call')).toHaveLength(1);
});
...
test('preserves existing config keys, comments, and user hooks', async () => {
  ...
  expect(getHookItems(doc, 'pre_tool_call')).toHaveLength(1);
  expect(getHookItems(doc, 'pre_llm_call')).toHaveLength(1);
...
test('re-running does not duplicate hooks', async () => {
  ...
  const doc = await readHermesConfig(tempDir);
  expect(getHookItems(doc, 'pre_llm_call')).toHaveLength(1);
  expect(getHookItems(doc, 'on_session_end')).toHaveLength(1);
  expect(getHookItems(doc, 'on_session_finalize')).toHaveLength(1);
});
