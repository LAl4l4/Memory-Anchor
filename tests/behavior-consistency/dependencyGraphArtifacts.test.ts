...
import { snapshotDirectory } from '../chart/testHelpers.ts';
...
test('full and incremental builds produce identical graphs and charts after a large cross-chart dependency switch', async () => {
  ...
  const makeLargeModule = (prefix: string, header: string): string => `${header}\n${Array.from(
    { length: 220 },
    (_, index) => `function ${prefix}Padding${index}() { return ${index}; }`
  ).join('\n')}\n`;

  const makeConsumer = (name: string, target: 'shared' | 'other'): string => {
  ...
  const writeWorkspace = async (projectRoot: string, updated: boolean): Promise<void> => {
  ...
  if (!provider || !consumers) {
    throw new Error('Expected provider and consumer directories in the initial chart tree');
  }
  const thresholds = {
  ...
  const callerPaths = (targetKey: string): string[] => (graph.reverseDependencies[targetKey] ?? [])
  ...
  const providerSymbolLines = fullArtifacts['chart/providers/chart.md']
    .split('\n')
