...
test('full and incremental builds produce identical artifacts after one batch adds virtual child directories', async () => {
  ...
  const seedWorkspace = async (projectRoot: string): Promise<void> => {
  ...
  const addedFiles: Array<[string, string]> = [
  ...
  const addFiles = async (projectRoot: string): Promise<void> => {
  ...
  if (!bench) throw new Error('Expected bench directory in the initial chart tree');
