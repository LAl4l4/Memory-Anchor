...
test('buildChartFull renders every fixture once in one architecture tree', async () => {
  ...
  expect(normalizedChart).toContain('## Architecture');
  expect(normalizedChart).toContain('### tests/chart/test-src/');
  for (const { file } of fixtures) {
    expect(normalizedChart.split('\n').filter(line => line === `- ${file}`)).toHaveLength(1);
  }
  expect(getNodeBlock(normalizedChart, 'tests/chart/test-src/sample.c')).toContain('- sample.c');
  expect(normalizedChart).not.toContain('## Directory Skeleton');
  expect(normalizedChart).not.toContain('## Symbols & Callers');
  expect(normalizedChart).not.toContain('dependencies:');
  expect(normalizedChart).not.toContain('dependencies: none');
  expect(normalizedChart).not.toContain('  - - ');
  expect(normalizedChart).not.toContain('  - + ');
});
...
test('chart uses in-chart Tree-sitter imports and emits symbol reverse dependencies', async () => {
  ...
  const consumer = getNodeBlock(chartContent, 'tests/chart/test-src/consumer.ts');
  expect(consumer).toContain('- consumer.ts -> tests/chart/test-src/dependency.ts');
  expect(chartContent.split('\n').filter(line => line.startsWith('- consumer.ts'))).toHaveLength(1);
...
test('symbol-free files retain one plain architecture entry', async () => {
  ...
  try {
    ...
    const chartContent = (await readFile(context.chartPath, 'utf8')).replace(/\\/g, '/');
    expect(getNodeBlock(chartContent, 'tests/chart/test-src/empty.ts')).toBe('- empty.ts');
  } finally {
...
test('reverse dependency lookup isolates same-named exports by resolved file path', async () => {
  ...
  expect(secondDependency).toContain(
    ...
    if (!secondDependency) throw new Error('Expected dependency-b chart node');
...
test('reverse dependencies group multiple callers from one source file', async () => {
  ...
  try {
    ...
    if (!dependency) throw new Error('Expected grouped-callers dependency node');
...
test('reverse dependencies distinguish duplicate caller names within one file', async () => {
  ...
  try {
    ...
    if (!dependency) throw new Error('Expected same-file dependency node');
...
test('forward dependencies resolve parseable files outside the rendered chart', async () => {
  ...
  expect(getNodeBlock(chart, 'src/consumer.ts')).toContain('- consumer.ts -> shared.ts');
  expect(getNodeBlock(chart, 'shared.ts')).toBeNull();
...
test('incremental updates rebuild reverse dependencies for the complete chart whitelist', async () => {
  ...
  const chartContent = (await readFile(context.chartPath, 'utf8')).replace(/\\/g, '/');
  expect(getNodeBlock(chartContent, 'tests/chart/test-src/consumer.ts')).toContain(
    '- consumer.ts -> tests/chart/test-src/dependency.ts'
  );
