...
test('a topology change in a mixed root batch leaves unrelated chart outputs untouched', async () => {
  ...
  const changingChars = assertDefined(
    initialRegistry.children.find(node => node.directory === 'changing'),
    'Expected changing directory in initial registry'
  ).thisDirectoryChars;
  ...
  const expandedRegistry = JSON.parse(
    await readFile(registryPath, 'utf8')
  ) as DirectoryTreeRegistryNode;
  const expandedChangingChars = assertDefined(
    expandedRegistry.children.find(node => node.directory === 'changing'),
    'Expected changing directory in expanded registry'
  ).thisDirectoryChars;
  ...
  expect(assertDefined(
    updatedRegistry.children.find(node => node.directory === 'changing'),
    'Expected changing directory in updated registry'
  ).isSplit)
    .toBe(true);
